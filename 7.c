...
#include <math.h>
#include "image.h"
#define IME_FAJLA "wall.bmp" /* Ime fajla sa teksturom */
...
static GLuint id_teksture[1]; /* Identifikator teksture */
...
static void initialize(void)
{
    ...
    image_read(image, IME_FAJLA);

    /* Generisu se identifikatori tekstura */
    glGenTextures(1, id_teksture);

    glBindTexture(GL_TEXTURE_2D, id_teksture[0]);
...
static void on_keyboard(unsigned char key, int x, int y)
{
    switch (key) {
		...
		case 'A':
			{
                /* Na taster A(a) pokreni animaciju */
        ...
        case 's':
        case 'S':
        /* Na taster S(s) zaustavi animaciju */
        animation_ongoing = 0;
        break;

        /* Klik tastera 1-9 odredjuje poziciju cekica */
		case '9':{
            cekicX = 8;
            cekicZ = 2;
            glutPostRedisplay();
...
//Materijali
    ...
    /* Postavlja se tekstura poligona*/
    glBindTexture(GL_TEXTURE_2D, id_teksture[0]);
    glBegin(GL_QUADS);
        glNormal3f(0, 1, 0); /* Y normala*/
